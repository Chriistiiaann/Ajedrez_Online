    ...
    useEffect(() => {
        ...
        const fetchFriendRequests = async () => {
            ...
            try {
                ...
                if (!response.ok) {
                    // Verificamos si el código de estado es 400 (Bad Request)
        ...
        // Llamamos a la función solo cuando el modal está abierto
        if (isOpen) {
            fetchFriendRequests()
        }
    }, [isOpen])
            ...
            <DialogContent className="text-white">
                ...
                <ul className="space-y-2">
                    {friendRequests.map((request) => (
                        <li key={request.requestId} className="flex items-center justify-between p-2 hover:bg-accent rounded-md">
